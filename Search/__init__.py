...
import pickle
import os
def nsga2_DE(measurer, representative_params, types, pop_size, num_generations, max_model_size, max_layers, save_file, freq_print=1, freq_save = 1):
    ...
    # Evaluate population
    population = evaluate_population(measurer, representative_params, population, max_model_size, max_layers)
    ...
    for i in range(num_generations):
        ...
        # Evaluate offspring
        offspring_population = evaluate_population(measurer, representative_params, offspring_population, max_model_size, max_layers)
        ...
        if (i+1)%freq_save==0:
            ...
            with open(os.path.join(save_file, 'evaluation_dict.pickle'), 'wb') as handle:
                pickle.dump(EVALUATON_DICT, handle, protocol=pickle.HIGHEST_PROTOCOL)
            with open(os.path.join(save_file, 'full_pop.pickle'), 'wb') as handle:
                pickle.dump(full_pop, handle, protocol=pickle.HIGHEST_PROTOCOL)
    return pareto_fronts

def nsga2_DE_resume(measurer, representative_params, population_str, pareto_fronts_str, types, pop_size, current_gen, num_generations, max_model_size, max_layers, save_file, freq_print=1, freq_save = 1):
    # Evaluate population
    population = [Individual(encode_str_structure(ind_str)) for ind_str in population_str]
    pareto_fronts = [[Individual(encode_str_structure(ind_str)) for ind_str in front] for front in pareto_fronts_str]
    population = evaluate_population(measurer, representative_params, population, max_model_size, max_layers)
    pareto_fronts = evaluate_paretos(measurer, representative_params, pareto_fronts, max_model_size, max_layers)

    for i in range(current_gen, num_generations):
        print(f"Iteration {i+1} of {num_generations}")
